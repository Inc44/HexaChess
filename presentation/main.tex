...
\definecolor{Purple-40}{HTML}{602CA0}
...
\begin{tikzpicture}[remember picture,overlay]
	\fill[Purple-40] (current page.south west) rectangle (current page.north east);
	\foreach \i in {0,...,12}
		\foreach \j in {0,...,8} {
...
\begin{frame}{Architecture technique}
	...
	\begin{tikzpicture}[node distance=1.5cm]
		...
		\draw[flow, <->] (ui) -- (logic);
		\draw[flow, ->] ([yshift=4mm]logic.east) -- node[above=4pt, font=\tiny, align=center, rotate=35] {\textbf{HTTP POST}\\JSON (Jackson)} ([yshift=4mm]server.west);
		\draw[flow, <->, dashed] ([yshift=-4mm]logic.east) -- node[below=4pt, font=\tiny, align=center, rotate=35] {\textbf{Polling}\\(Sync Move)} ([yshift=-4mm]server.west);
...
\begin{frame}{Conclusion}
	...
	\vspace{0.5cm}
	\textcolor{Purple-40}{\textbf{HexaChess}} est prêt pour la démonstration.
